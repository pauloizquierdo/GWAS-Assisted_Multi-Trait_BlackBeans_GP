...
rm(list=ls())
setwd('/mnt/gs21/scratch/izquier7/r_bbl/year_2018')
...
JOBS <- expand.grid(rep=1:100, trait=1)
...
X <- as.matrix(chapter3_data$nirs18)
#X <- as.matrix(chapter3_data$nirs19)
...
accuracy_KA18 = cor(y[tst],
                         fmKA$yHat[tst],
                         use = 'pairwise.complete.obs')

### Multi-trait GBLUP - SSI
...
accuracy_GBLUP_MT18 = cor(y[tst],fm_muv$ETAHat[tst])

### Multi-trait GBLUP - SW
Y_sw <- cbind(yNA,sw)

fm_musw <- Multitrait(y=Y_sw, ETA=ETA, 
                     resCov=list(type="DIAG"), 
                     nIter=nIter, burnIn=burnIn)


accuracy_GBLUP_MTSW18 = cor(y[tst],fm_musw$ETAHat[tst])
...
accuracy_KA_MT18 = cor(y[tst],fm_muv_ka$ETAHat[tst])

### Multi-trait KA - SW
fm_muv_ka_sw <- Multitrait(y=Y_sw, ETA=KList, 
                        resCov=list(type="DIAG"), 
                        nIter=nIter, burnIn=burnIn)

accuracy_KA_MTSW18 = cor(y[tst],fm_muv_ka_sw$ETAHat[tst])
...
if (length(indexQTL) >= 1) {
  ...
  fm_gwas <- BGLR(y=yNA, ETA=ETA, nIter=nIter, burnIn=burnIn)
  ...
  accuracy_gblup_mt_gwas18 = cor(y[tst],fm_gblup_gwas_mt$ETAHat[tst])
  
  # GBLUP + GWAS + MT-SW
  
  fm_gblup_gwas_mtsw <- Multitrait(y=Y_sw, ETA=ETA, 
                                 resCov=list(type="DIAG"), 
                                 nIter=nIter, burnIn=burnIn)
  
  
  accuracy_gblup_mt_gwas18sw = cor(y[tst],fm_gblup_gwas_mtsw$ETAHat[tst])
  ...
  accuracy_KA_gwas_mt18 = cor(y[tst],fm_muv_ka_gwas_mt$ETAHat[tst])
  
  #### KA + GWAS + MT - SW
  fm_muv_ka_gwas_mtsw <- Multitrait(y=Y_sw, ETA=ETA, 
                                   resCov=list(type="DIAG"), 
                                   nIter=nIter, burnIn=burnIn)
  
  
  accuracy_KA_gwas_mt18sw = cor(y[tst],fm_muv_ka_gwas_mtsw$ETAHat[tst])
  
} else {
  accuracy_gwas = 'NA'
  accuracy_KA_gwas18 = 'NA'
  accuracy_gblup_mt_gwas18 = 'NA'
  accuracy_gblup_mt_gwas18sw = 'NA'
  accuracy_KA_gwas_mt18 = 'NA'
  accuracy_KA_gwas_mt18sw = 'NA'
}
...
gp_results <- c(accuracy, 
                ...
                accuracy_KA18, 
                accuracy_GBLUP_MT18, 
                accuracy_GBLUP_MTSW18,
                accuracy_KA_MT18,
                accuracy_KA_MTSW18,
                accuracy_gwas, 
                accuracy_gblup_mt_gwas18,
                accuracy_gblup_mt_gwas18sw,
                accuracy_KA_gwas18, 
                accuracy_KA_gwas_mt18,
                accuracy_KA_gwas_mt18sw)

names(gp_results) <- c('SI','SSI','GBLUP','KA','GBLUP_MT',
                       'GBLUP_MT-SW', 'KA_MT', 'KA_MT-SW', 
                       'GBLUP_GWAS', 'GBLUP_GWAS_MT',
                       'GBLUP_GWAS_MT-SW',
                       'KA_GWAS','KA_GWAS_MT',
                       'KA_GWAS_MT-MT')

save(gp_results,
     file=paste0("results_cor_",trait_name,"_rep_",rep,
                 ".RData"))

save(GWAS_res,
     file=paste0("GWAS_results_cor_",trait_name,"_rep_",
                 rep,".RData"))