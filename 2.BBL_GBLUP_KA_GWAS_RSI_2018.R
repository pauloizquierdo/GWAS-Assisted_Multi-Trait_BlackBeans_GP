...
index <- 1:272 # first breding cycle samples
M1 <- M[index,]
GD <- myGD[index,]
G1 <- G[index,index]
D1 <- D[index,index]
...
accuracy_gwas = c()
accuracy_KA_gwas18 = c()
accuracy_KA_gwas18 = c()
accuracy_KA_gwas_mt18 = c()
...
if (length(indexQTL) >= 1) {
  ...
  index <- 1:272 # first breeding cycle samples
  M_new <- M_new[index,]
  QTL <- QTL[index,]
...
gp_results <- c(accuracy, 
                accuracy_GBLUP18, accuracy_KA18, 
                accuracy_GBLUP_MT18, accuracy_KA_MT18,
                accuracy_gwas, accuracy_gblup_mt_gwas18,
                accuracy_KA_gwas18, accuracy_KA_gwas_mt18)
