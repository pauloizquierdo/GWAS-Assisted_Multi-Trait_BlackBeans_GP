...
trait_name <- colnames(Y18)[trait]
y18 <- Y18[,trait]

index <- which(!is.na(y18))
M1 <- M1[index,]
G1 <- G1[index,index]
D1 <- D1[index,index]
y18 <- y18[index]
...
accuracy_gs_tst_18 = cor(y18[tst],
                         fm_uni$yHat[tst],
...
accuracy_KA_tst_18 = cor(y18[tst],
                         fmKA$yHat[tst],
                         use = 'pairwise.complete.obs')

### results
gp_results <- c(accuracy_gs_tst_18, accuracy_Kernel,
                accuracy_KA_tst_18)
